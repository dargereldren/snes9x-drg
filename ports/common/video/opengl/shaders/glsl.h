...
#include "snes9x.h"
#include "video/vulkan/slang_preset_ini.hpp"
...
typedef void (*GLSLViewportCallback)(int source_width, int source_height, int viewport_x, int viewport_y, int viewport_width, int viewport_height, int *out_dst_x, int *out_dst_y, int *out_dst_width, int *out_dst_height);
...
struct GLSLShader {
	...
	void read_shader_file_with_includes(const std::string &filename, std::vector<std::string> &lines, int p);
	GLuint compile_shader(std::vector<std::string> &lines, const char *aliases, const char *defines, GLuint type, GLuint *out);
	...
	bool using_slang;
#ifdef USE_SLANG
	std::string slang_get_stage(std::vector<std::string> &lines, const std::string &name);
