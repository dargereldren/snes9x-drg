...
void S9xPortAudioSoundDriver::init() {
	...
	if (err != paNoError) {
		fprintf(stderr, "Couldn't initialize PortAudio: %s\n", Pa_GetErrorText(err));
	}
}
...
bool S9xPortAudioSoundDriver::tryHostAPI(int index) {
	...
	printf("(%s : %s, latency %dms)...\n", hostapi_info->name, device_info->name, (int)(param.suggestedLatency * 1000.0));

	auto err = Pa_OpenStream(&audio_stream, nullptr, &param, playback_rate, 0, paNoFlag, nullptr, nullptr);
