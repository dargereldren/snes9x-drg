...
#ifdef NP_DEBUG
		printf("SERVER: Messages lost from '%s', expected %d, got %d\n", NPServer.Clients[c].HostName ? NPServer.Clients[c].HostName : "Unknown", NPServer.Clients[c].ReceiveSequenceNum, header[1]);
#endif
...
void S9xNPServerLoop(void *) {
	...
	while (server_continue) {
		...
		if (timercmp(&next1, &now, >)) {
			...
			unsigned timeleft =
				(next1.tv_sec - now.tv_sec) * 1000000 + next1.tv_usec - now.tv_usec;
...
bool8 S9xNPSendROMImageToClient(int c) {
	...
	*ptr++ = Memory.HiROM;
	...
	if (!S9xNPSSendData(NPServer.Clients[c].Socket, header, sizeof(header)) ||
		...
		!S9xNPSSendData(NPServer.Clients[c].Socket, (uint8 *)Memory.ROMFilename.c_str(), Memory.ROMFilename.length() + 1)) {
		S9xNPShutdownClient(c, TRUE);
		return (FALSE);
	}
	return (TRUE);
...
void S9xNPSendSRAMToClient(int c) {
	...
	*ptr++ = NP_SERV_SRAM_DATA;
	...
	if (!S9xNPSSendData(NPServer.Clients[c].Socket, sram, sizeof(sram)) ||
		(len > 7 &&
		 !S9xNPSSendData(NPServer.Clients[c].Socket, Memory.SRAM, len - 7))) {
		S9xNPShutdownClient(c, TRUE);
	}
}
