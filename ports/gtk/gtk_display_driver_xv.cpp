...
void S9xXVDisplayDriver::create_window(int width, int height) {
	...
	gdk_window = gdk_window_new(drawing_area->get_window()->gobj(), &window_attr, GDK_WA_X | GDK_WA_Y | GDK_WA_VISUAL);
...
void S9xXVDisplayDriver::update(uint16_t *buffer, int width, int height, int stride_in_pixels) {
	...
	if (format == FOURCC_YUY2) {
		S9xConvertYUV(buffer, (uint8 *)xv_image->data, stride_in_pixels * 2, 2 * xv_image->width, width + (width < xv_image->width ? (width % 2) + 4 : 0), height + (height < xv_image->height ? 4 : 0));
	} else {
		S9xConvertMask(buffer, (uint8 *)xv_image->data, stride_in_pixels * 2, bytes_per_pixel * xv_image->width, width + (width < xv_image->width ? (width % 2) + 4 : 0), height + (height < xv_image->height ? 4 : 0), rshift, gshift, bshift, bpp);
	}
	...
	XvShmPutImage(display, xv_portid, xwindow, XDefaultGC(display, XDefaultScreen(display)), xv_image, 0, 0, width, height, dst.x, dst.y, dst.w, dst.h, False);
...
void S9xXVDisplayDriver::update_image_size(int width, int height) {
	if (xv_image_width != width || xv_image_height != height) {
		...
		xv_image = XvShmCreateImage(display, xv_portid, format, 0, width, height, &shm);
...
int S9xXVDisplayDriver::init() {
	...
	xv_portid = -1;
	if ((XvQueryAdaptors(display, gdk_x11_window_get_xid(root), &num_adaptors, &adaptors)) != Success) {
	...
	for (int i = 0; i < (int)num_adaptors; i++) {
		if (adaptors[i].type & XvInputMask &&
			...
			formats = XvListImageFormats(display, adaptors[i].base_id, &num_formats);
	...
	formats = XvListImageFormats(display, xv_portid, &num_formats);
	...
	xcolormap = XCreateColormap(display, gdk_x11_window_get_xid(drawing_area->get_window()->gobj()), vi->visual, AllocNone);

	create_window(1, 1);
	gdk_window_hide(gdk_window);

	/* Allocate a shared memory image. */
	xv_image = XvShmCreateImage(display, xv_portid, format, 0, 512, 512, &shm);
...
int S9xXVDisplayDriver::query_availability() {
	...
	if (XvQueryExtension(display, &p_version, &p_release, &p_request_base, &p_event_base, &p_error_base) != Success) {
		return 0;
	}
