...
bool S9xOSSSoundDriver::open_device(int playback_rate, int buffer_size_ms) {
	...
	printf("    --> (Buffer size: %d bytes, %dms latency)...", output_buffer_size_bytes, (output_buffer_size_bytes * 250) / playback_rate);
...
bool S9xOSSSoundDriver::write_samples(int16_t *data, int samples) {
	...
	while (bytes_to_write > bytes_written) {
		...
		int result = write(filedes, ((char *)data) + bytes_written, bytes_to_write - bytes_written);
