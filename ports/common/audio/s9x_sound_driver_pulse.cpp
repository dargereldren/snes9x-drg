...
bool S9xPulseSoundDriver::open_device(int playback_rate, int buffer_size_ms) {
	...
	S9xMessage(S9X_INFO, S9X_NO_INFO, fmt::format("    --> ({0:Ld} Hz, 16‑bit stereo, {1:Ld} ms)…", playback_rate, buffer_size_ms).c_str());
	...
	if (pa_stream_connect_playback(stream, nullptr, &buffer_attr, PA_STREAM_EARLY_REQUESTS, nullptr, nullptr) < 0) {
		return false;
	}
