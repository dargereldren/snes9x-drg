...
std::string GLSLShader::slang_get_stage(std::vector<std::string> &lines, const std::string &name) {
...
GLint GLSLShader::slang_compile(std::vector<std::string> &lines, const std::string &stage) {
	...
	if (!shader.preprocess(GetDefaultResources(), 100, ENoProfile, false, false, messages, &debug, forbid_includer)) {
		puts(debug.c_str());
		return -1;
	}
...
void GLSLShader::slang_introspect() {
	...
	for (int i = 1; i < (int)pass.size(); i++) {
		...
		if (uniform_block) {
			glGetActiveUniformBlockiv(p.program, 0, GL_UNIFORM_BLOCK_DATA_SIZE, &uniform_block_size);
