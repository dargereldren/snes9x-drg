...
#ifdef DEBUGGER
	S9xTraceFormattedMessage("--- IRQ Timer HC:%d VC:%d set %d cycles HTimer:%d Pos:%04d->%04d  VTimer:%d Pos:%03d->%03d", CPU.Cycles, CPU.V_Counter, Timings.NextIRQTimer, PPU.HTimerEnabled, PPU.IRQHBeamPos, PPU.HTimerPosition, PPU.VTimerEnabled, PPU.IRQVBeamPos, PPU.VTimerPosition);
#endif
}
...
void S9xSetPPU(uint8 Byte, uint16 Address) {
	...
	if (Settings.MSU1 && (Address & 0xfff8) == 0x2000) { // MSU-1
		...
		if (Settings.SuperFX && SuperFX.isFx4 && Address >= 0x3000 && Address <= 0x3fff) {
			// FX4: registers $3000-$303F + 4KB cache $3040-$3FFF
			S9xSetSuperFX(Byte, Address);
			return;
		} else if (Settings.SuperFX && !SuperFX.isFx3 && !SuperFX.isFx4 && Address >= 0x3000 && Address <= 0x32ff) {
			S9xSetSuperFX(Byte, Address);
			return;
		} else if (Settings.SuperFX && SuperFX.isFx3 && Address >= 0x7000) {
...
uint8 S9xGetCPU(uint16 Address) {
	if (Address < 0x4200) {
		...
		switch (Address & 0xf) {
		case 0x0: // 0x43x0: DMAPx
			return ((DMA[d].ReverseTransfer ? 0x80 : 0) | (DMA[d].HDMAIndirectAddressing ? 0x40 : 0) | (DMA[d].UnusedBit43x0 ? 0x20 : 0) | (DMA[d].AAddressDecrement ? 0x10 : 0) | (DMA[d].AAddressFixed ? 0x08 : 0) | (DMA[d].TransferMode & 7));
