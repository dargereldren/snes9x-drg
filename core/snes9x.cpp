...
static bool parse_controller_spec(int port, const char *arg) {
	...
	} else if (!strncasecmp(arg, "mp5:", 4) && ((arg[4] >= '1' && arg[4] <= '8') || arg[4] == 'n') && ((arg[5] >= '1' && arg[5] <= '8') || arg[5] == 'n') && ((arg[6] >= '1' && arg[6] <= '8') || arg[6] == 'n') && ((arg[7] >= '1' && arg[7] <= '8') || arg[7] == 'n') && arg[8] == '\0') {
		S9xSetController(port, CTL_MP5, (arg[4] == 'n') ? -1 : arg[4] - '1', (arg[5] == 'n') ? -1 : arg[5] - '1', (arg[6] == 'n') ? -1 : arg[6] - '1', (arg[7] == 'n') ? -1 : arg[7] - '1');
	} else {
		return (false);
	}
			...
			if (!strcasecmp(argv[i], "-help")) {
				...
				if (!strcasecmp(argv[i], "-soundsync")) {
					...
					if (!strcasecmp(argv[i], "-displaytime")) {
						...
						if (!strcasecmp(argv[i], "-nomp5")) {
						...
						} else if (!strcasecmp(argv[i], "-nomacsrifle")) {
							...
							if (!strcasecmp(argv[i], "-hirom")) {
							...
							} else if (!strcasecmp(argv[i], "-snapshot")) {
								if (i + 1 < argc) {
								...
								} else if (!strcasecmp(argv[i], "-gamegenie") || !strcasecmp(argv[i], "-actionreplay") || !strcasecmp(argv[i], "-cheatcode")) {
									if (i + 1 < argc) {
										if (S9xAddCheatGroup("Unknown", argv[++i]) < 0) {
											S9xMessage(S9X_ERROR, S9X_GAME_GENIE_CODE_ERROR, "Code format invalid");
										} else {
								...
								// HACKING OR DEBUGGING OPTIONS
									...
									ENSURE_TRACE_OPEN(trace, "trace.log", "wb")
									CPU.Flags |= TRACE_FLAG;
								} else if (!strcasecmp(argv[i], "-dumpops")) {
