	...
	class ConfigEntry {
	  protected:
		...
		struct line_less {
			bool operator()(const ConfigEntry &a, const ConfigEntry &b) const {
				if (a.line == b.line) {
					return (b.val.empty() && !a.val.empty()) || a.key < b.key;
				}
				if (b.line < 0) {
					return true;
				}
		...
		static void trim(std::string &s) {
			...
			if (i > 0) {
				s.erase(0, i); // erase leading whitespace
			}
			i = s.find_last_not_of(" \f\n\r\t\v");
			if (i != -1) {
				s.erase(i + 1); // erase trailing whitespace
			}
			return;
		}
	  ...
	  public:
		ConfigEntry(int l, const std::string &s, const std::string &k, const std::string &v) : line(l), section(s), key(k), val(v) {
			trim(section);
			trim(key);
			used = false;
		}
	  ...
	  public:
		uint32 GetSectionSize(const std::string section) {
			...
			for (it = sections.begin(); it != sections.end(); it++) {
				...
				if (it->first == section || !section.compare(0, seclen, it->first, 0, seclen)) {
					count += it->second;
				}
			}
			return count;
