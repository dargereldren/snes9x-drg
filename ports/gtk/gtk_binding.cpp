...
Gdk::ModifierType Binding::get_gdk_modifiers() {
	return (Gdk::ModifierType)(((BINDING_CTRL & value) ? Gdk::CONTROL_MASK : 0) | ((BINDING_ALT & value) ? Gdk::MOD1_MASK : 0) | ((BINDING_SHIFT & value) ? Gdk::SHIFT_MASK : 0));
}
...
#define maybegettext(String) translate ? gettext(String) : (String)
	...
	if (is_key()) {
		...
		str = fmt::format(fmt::runtime(maybegettext("Keyboard {}{}{}{}")), (value & BINDING_SHIFT) ? "Shift+" : "", (value & BINDING_CTRL) ? "Ctrl+" : "", (value & BINDING_ALT) ? "Alt+" : "", str);
	}

	else if (is_joy()) {
		if ((get_key()) >= 512) {
			str = fmt::format(fmt::runtime(maybegettext("Axis {} {} {}%")), get_axis(), is_positive() ? "+" : "-", get_threshold());
		} else {
