		...
		for (auto &uniform : pipeline.shader->uniforms) {
			...
			case SlangShader::Uniform::PassFeedbackSize: // TODO: Does this need to differ?
				...
				if (uniform.specifier == -1) {
					...
					write_size(pipelines[uniform.specifier]->destination_width, pipelines[uniform.specifier]->destination_height);
				}
...
bool ShaderChain::load_shader_preset(const std::string &filename) {
	...
	float vertex_data[] = {-1.0f, -3.0f, 0.0f, 1.0f, /* texcoords */ 0.0, -1.0f, 3.0f, 1.0f, 0.0f, 1.0f, 2.0f, 1.0f, -1.0f, 1.0f, 0.0f, 1.0f, 0.0f, 1.0f};
...
bool ShaderChain::do_frame_without_swap(uint8_t *data, int width, int height, int stride, vk::Format format, int viewport_x, int viewport_y, int viewport_width, int viewport_height) {
	...
	for (size_t i = 0; i < pipelines.size(); i++) {
		...
		if (preset->last_pass_uses_feedback && is_last_pass) {
			image_layout_transition(cmd, frame.image.image, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal);
			image_layout_transition(cmd, context->swapchain->get_image(), vk::ImageLayout::ePresentSrcKHR, vk::ImageLayout::eTransferSrcOptimal);
			...
			image_layout_transition(cmd, frame.image.image, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal);
			image_layout_transition(cmd, context->swapchain->get_image(), vk::ImageLayout::eTransferSrcOptimal, vk::ImageLayout::ePresentSrcKHR);
...
void ShaderChain::upload_original(vk::CommandBuffer cmd, uint8_t *data, int width, int height, int stride, vk::Format format) {
	...
	if (create_texture) {
		texture->create(width, height, format, wrap_mode_from_string(pipelines[0]->shader->wrap_mode), pipelines[0]->shader->filter_linear, pipelines[0]->shader->mipmap_input);
	}
