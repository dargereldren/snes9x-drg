...
#ifdef DEBUGGER
	if (Settings.TraceHCEvent) {
		S9xTraceFormattedMessage("--- HC event processing  (%s)  expected HC:%04d  executed HC:%04d VC:%04d", eventname[CPU.WhichEvent], CPU.NextEvent, CPU.Cycles, CPU.V_Counter);
	}
#endif
...
#ifdef DEBUGGER
	if (Settings.TraceHCEvent) {
		S9xTraceFormattedMessage("--- HC event rescheduled (%s)  expected HC:%04d  current  HC:%04d", eventname[CPU.WhichEvent], CPU.NextEvent, CPU.Cycles);
	}
#endif
}
