...
bool SlangPreset::introspect_shader(SlangShader &shader, int pass, SlangShader::Stage stage) {
	...
	if (res.push_constant_buffers.empty()) {
		...
		for (size_t i = 0; i < pcb_type.member_types.size(); i++) {
			...
			if (match_buffer_semantic(name, pass, semantic_type, specifier)) {
				SlangShader::Uniform uniform{SlangShader::Uniform::Block::PushConstant, offset, semantic_type, specifier};
	...
	if (res.uniform_buffers.empty()) {
		...
		for (size_t i = 0; i < ubo_type.member_types.size(); i++) {
			...
			int specifier;
			if (match_buffer_semantic(name, pass, semantic_type, specifier)) {
				SlangShader::Uniform uniform{SlangShader::Uniform::Block::UBO, offset, semantic_type, specifier};
