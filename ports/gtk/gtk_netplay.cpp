...
static void S9xNetplayConnect() {
	...
	if (!S9xNPConnectToServer(gui_config->netplay_last_host.c_str(), gui_config->netplay_last_port, Memory.ROMName)) {
		msg = gtk_message_dialog_new(NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "Couldn't connect to server: %s:%d", gui_config->netplay_last_host.c_str(), gui_config->netplay_last_port);
...
void S9xNetplayStartServer() {
	...
	S9xNPConnectToServer("127.0.0.1", gui_config->netplay_default_port, Memory.ROMName);
...
void S9xNetplayDialogOpen() {
	...
	gtk_window_set_transient_for(np_dialog->get_window(), top_level->get_window());
