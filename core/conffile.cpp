...
void ConfigFile::LoadFile(Stream *r, const char *name) {
	...
	do {
		...
		if (val.size() > 0 && val[0] == '"' && *val.rbegin() == '"') {
...
#define section_then_key_less key_less
	...
	for (set<ConfigEntry, ConfigEntry::section_then_key_less>::iterator j = data2.begin();; j++) {
		if (j == data2.end() || j->section != section) {
			if (!tmp.empty()) {
				fprintf(fp, "\n[%s]\n", section.c_str());
				unsigned int maxKeyLen = 0, maxValLen = 0;
				int maxLeftDiv = 0;
				int maxRightDiv = -1;
				if (niceAlignment) {
				...
				for (set<ConfigEntry, ConfigEntry::line_less>::iterator i = tmp.begin(); i != tmp.end(); i++) {
					...
					if (niceAlignment) {
						...
						int len3sub = 0;
						if (len3 < maxRightDiv) {
							for (int j = len3; j < maxRightDiv; j++) {
								fputc(' ', fp);
							}
						...
						for (unsigned int j = i->key.length() + len3 + 3; j < maxKeyLen; j++) {
							fputc(' ', fp);
						}
						fprintf(fp, "%s", i->key.c_str());
						for (int j = 0; j < len3 - len3sub; j++) {
							fputc(' ', fp);
						}
					...
					if (showComments && !i->comment.empty()) {
						if (niceAlignment) {
							for (unsigned int j = o.length(); j < maxValLen; j++) {
								fputc(' ', fp);
							}
						}
...
const char *ConfigFile::GetString(const char *key, const char *def) {
	...
	if (i == data.end()) {
		if (defaultAutoAdd) {
			SetString(key, ""); // SetString(key, def?def:"");
		}
		return def;
	}
	i->used = true;
...
bool ConfigFile::GetBool(const char *key, bool def, bool *bad) {
	...
	if (!strcasecmp(c, "true") || !strcasecmp(c, "1") || !strcasecmp(c, "yes") || !strcasecmp(c, "on")) {
		return true;
	}
	if (!strcasecmp(c, "false") || !strcasecmp(c, "0") || !strcasecmp(c, "no") || !strcasecmp(c, "off")) {
		return false;
	}
	if (bad) {
		*bad = true;
