...
S9xRect S9xApplyAspect(int src_width, int src_height, int dst_width, int dst_height) {
	...
	} else if (gui_config->maintain_aspect_ratio && !(screen_aspect <= snes_aspect * (1.0 + granularity) && screen_aspect >= snes_aspect * (1.0 - granularity))) {
...
static void internal_convert_16_yuv(void *src_buffer, void *dst_buffer, int src_pitch, int dst_pitch, int width, int height, int bpp) {
...
static void internal_convert_mask(void *src_buffer, void *dst_buffer, int src_pitch, int dst_pitch, int width, int height, unsigned int inv_rshift, unsigned int inv_gshift, unsigned int inv_bshift, unsigned int bpp) {
	switch (bpp) {
	case 15:
	case 16:
		for (int y = 0; y < height; y++) {
...
static void internal_convert(void *src_buffer, void *dst_buffer, int src_pitch, int dst_pitch, int width, int height, int bpp) {
...
static void internal_threaded_convert(void *src_buffer, void *dst_buffer, int src_pitch, int dst_pitch, int width, int height, int bpp) {
	create_thread_pool();
	...
	for (int i = 0; i < gui_config->num_threads; i++) {
		...
		pool->queue([=] {
			func((uint8 *)src_buffer + (src_pitch * lines_handled), (uint8 *)dst_buffer + (dst_pitch * lines_handled), src_pitch, dst_pitch, width, job_height, bpp);
		});
...
static void internal_threaded_convert_mask(void *src_buffer, void *dst_buffer, int src_pitch, int dst_pitch, int width, int height, int inv_rmask, int inv_gmask, int inv_bmask, int bpp) {
	create_thread_pool();
	...
	for (int i = 0; i < gui_config->num_threads; i++) {
		...
		pool->queue([=] {
			internal_convert_mask((uint8 *)src_buffer + (src_pitch * lines_handled), (uint8 *)dst_buffer + (dst_pitch * lines_handled), src_pitch, dst_pitch, width, job_height, inv_rmask, inv_gmask, inv_bmask, bpp);
		});
		lines_handled += job_height;
	}
...
void S9xConvertYUV(void *src_buffer, void *dst_buffer, int src_pitch, int dst_pitch, int width, int height) {
	if (gui_config->multithreading) {
		internal_threaded_convert(src_buffer, dst_buffer, src_pitch, dst_pitch, width, height, -1);
	} else {
		internal_convert_16_yuv(src_buffer, dst_buffer, src_pitch, dst_pitch, width, height, -1);
	}
}

void S9xConvert(void *src, void *dst, int src_pitch, int dst_pitch, int width, int height, int bpp) {
	if (gui_config->multithreading) {
		internal_threaded_convert(src, dst, src_pitch, dst_pitch, width, height, bpp);
	} else {
		internal_convert(src, dst, src_pitch, dst_pitch, width, height, bpp);
	}
}
...
void S9xConvertMask(void *src, void *dst, int src_pitch, int dst_pitch, int width, int height, int rshift, int gshift, int bshift, int bpp) {
	if (gui_config->multithreading) {
		internal_threaded_convert_mask(src, dst, src_pitch, dst_pitch, width, height, rshift, gshift, bshift, bpp);
	} else {
		internal_convert_mask(src, dst, src_pitch, dst_pitch, width, height, rshift, gshift, bshift, bpp);
	}
}
...
void S9xQueryDrivers() {
	...
	if (is_x11()) {
		...
		if (!gui_config->xrr_crtc_info) {
			gui_config->xrr_crtc_info = XRRGetCrtcInfo(dpy, gui_config->xrr_screen_resources, gui_config->xrr_screen_resources->crtcs[0]);
		}
	}
#endif
...
bool8 S9xDeinitUpdate(int width, int height) {
	...
	if (top_level->last_height > height) {
		memset(GFX.Screen + GFX.RealPPL * height, 0, GFX.Pitch * (top_level->last_height - height));
	}
