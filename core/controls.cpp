...
char *S9xGetCommandName(s9xcommand_t command) {
	...
	case S9xButtonMouse:
		...
		if (command.button.mouse.left) {
			s += "L";
		}
		if (command.button.mouse.right) {
			s += "R";
		}
	...
	case S9xButtonSuperscope:
		...
		if (command.button.scope.aim_offscreen) {
			s += " AimOffscreen";
		}
	...
	case S9xButtonJustifier:
		...
		if (command.button.justifier.aim_offscreen) {
			s += " AimOffscreen";
		}
	...
	case S9xPointer:
		...
		if (command.pointer.aim_macsrifle) {
			s += c;
			s += "MacsRifle";
			c = '+';
		}
	...
	case S9xButtonPseudopointer:
		...
		if (command.button.pointer.UD) {
			s += (command.button.pointer.UD == 1) ? 'd' : 'u';
		}
		if (command.button.pointer.LR) {
			s += (command.button.pointer.LR == 1) ? 'r' : 'l';
		}
	...
	case S9xAxisPseudopointer:
		...
		if (command.axis.pointer.invert) {
			s += "-";
		}
	...
	case S9xButtonMulti: {
		...
		s = "{";
		if (multis[command.button.multi_idx]->multi_press) {
			s = "+{";
		}
...
s9xcommand_t S9xGetCommandT(const char *name) {
	...
	if (!strcmp(name, "None")) {
		...
		if (!strncmp(name + 8, "Axis ", 5)) {
			...
			if ((cmd.button.joypad.toggle = strncmp(s, "Toggle", 6) ? 0 : 1)) {
				s += i = 6;
			}
			if ((cmd.button.joypad.sticky = strncmp(s, "Sticky", 6) ? 0 : 1)) {
				s += i = 6;
			}
			if ((cmd.button.joypad.turbo = strncmp(s, "Turbo", 5) ? 0 : 1)) {
				s += i = 5;
			}
		...
		if ((cmd.button.mouse.left = (*s == 'L'))) {
			s += i = 1;
		}
		if ((cmd.button.mouse.right = (*s == 'R'))) {
			s += i = 1;
		}
	...
	} else if (!strncmp(name, "MacsRifle ", 10)) {
		s = name + 10;
		i = 0;
		...
		if ((cmd.pointer.aim_justifier1 = strncmp(s, "Justifier2", 10) ? 0 : 1)) {
			s += i = 10;
			if (*s == '+') {
				s++;
			}
		...
		if ((cmd.button.pointer.UD = (*s == 'u' ? -1 : (*s == 'd' ? 1 : 0)))) {
			s += i = 1;
		}
		if ((cmd.button.pointer.LR = (*s == 'l' ? -1 : (*s == 'r' ? 1 : 0)))) {
			s += i = 1;
		}
...
bool S9xMapPointer(uint32 id, s9xcommand_t mapping, bool poll) {
	...
	if (poll) {
		if (id >= PseudoPointerBase) {
			...
			switch (mapping.type) {
			case S9xPointer:
				if (mapping.pointer.aim_mouse0) {
					pollmap[MOUSE0].insert(id);
				}
				if (mapping.pointer.aim_mouse1) {
					pollmap[MOUSE1].insert(id);
				}
				if (mapping.pointer.aim_scope) {
					pollmap[SUPERSCOPE].insert(id);
				}
				if (mapping.pointer.aim_justifier0) {
					pollmap[ONE_JUSTIFIER].insert(id);
				}
				if (mapping.pointer.aim_justifier1) {
					pollmap[TWO_JUSTIFIERS].insert(id);
				}
				if (mapping.pointer.aim_macsrifle) {
					pollmap[MACSRIFLE].insert(id);
				}
				break;
...
void S9xApplyCommand(s9xcommand_t cmd, int16 data1, int16 data2) {
	...
	case S9xButtonJoypad:
		if (cmd.button.joypad.toggle) {
			...
			if (cmd.button.joypad.turbo) {
				joypad[cmd.button.joypad.idx].toggleturbo ^= r;
			}
			if (cmd.button.joypad.sticky) {
				joypad[cmd.button.joypad.idx].togglestick ^= r;
			}
		} else {
			uint16 r, s, t, st;
	...
	case S9xButtonMouse:
		i = 0;
		if (cmd.button.mouse.left) {
			i |= 0x40;
		}
		if (cmd.button.mouse.right) {
			i |= 0x80;
		}
	...
	case S9xButtonSuperscope:
		i = 0;
		if (cmd.button.scope.fire) {
			i |= SUPERSCOPE_FIRE;
		}
		if (cmd.button.scope.cursor) {
			i |= SUPERSCOPE_CURSOR;
		}
		if (cmd.button.scope.pause) {
			i |= SUPERSCOPE_PAUSE;
		}
		if (cmd.button.scope.aim_offscreen) {
	...
	case S9xButtonJustifier:
		i = 0;
		if (cmd.button.justifier.trigger) {
			i |= JUSTIFIER_TRIGGER;
		}
		if (cmd.button.justifier.start) {
			i |= JUSTIFIER_START;
		}
		if (cmd.button.justifier.aim_offscreen) {
			justifier.offscreen[cmd.button.justifier.idx] = data1 ? 1 : 0;
		}
	...
	case S9xButtonMacsRifle:
		i = 0;
		if (cmd.button.macsrifle.trigger) {
			i |= MACSRIFLE_TRIGGER;
		}
...
#define COPY(x)                                                \
	{                                                          \
		memcpy((char *)s->internal_macs + i, &(x), sizeof(x)); \
		i += sizeof(x);                                        \
	}
	i = 0;
...
#define COPY(x)                                                \
	{                                                          \
		memcpy(&(x), (char *)s->internal_macs + i, sizeof(x)); \
		i += sizeof(x);                                        \
	}
			i = 0;
...
bool MovieGetMacsRifle(int i, uint8 out[5]) {
	...
	WRITE_WORD(ptr, macsrifle.x);
	ptr += 2;
	WRITE_WORD(ptr, macsrifle.y);
	ptr += 2;
...
void MovieSetMacsRifle(int i, uint8 in[5]) {
	...
	macsrifle.x = READ_WORD(ptr);
	ptr += 2;
	macsrifle.y = READ_WORD(ptr);
	ptr += 2;
